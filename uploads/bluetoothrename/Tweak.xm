...
{
    if([[UIApplication sharedApplication]isKingOfClass:objc_getClass("SpringBoard")]==YES)
    {
...
-(void)layoutSubviews{
      %orig; 
      if([[devices allValues] count]!= 0 && self == self.tableCell.textLabel && [self.tableCell isKindOfClass: objc_getClass("BTTableCell")] == YES && [[UIApplication sharedApplication]isKindOfClass:[objc_getClass("PreferencesAppController") class]] ==YES)
      {
...
-(void)beginEditName{
      if ([field superview]!= self.tableCell && [[UIApplication sharedApplication]isKindOfClass:[objc_getClass("PreferencesAppController")  class]] ==YES)
      {
          for (int i=0; i<[[devices allKeys] count]; i++)
               {
                      if ([self.text isEqualToString:[[[devices allValues] objectAtIndex:i]objectForKey:@"Name"]]==YES)
...
-(BOOL)resignFirstResponder{
      %orig;   
       if([[UIApplication sharedApplication]isKindOfClass:[objc_getClass("PreferencesAppController")  class]] ==YES)
       {
...
-(void)_updateWithItems: (id) arg1 updateSupport: (id) arg2 {
       if(field != nil && [[UIApplication sharedApplication]isKindOfClass:[objc_getClass("PreferencesAppController")  class]] ==YES )
       {
